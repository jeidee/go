...
import (
	...
	"github.com/jeidee/go/go-net/example/protobuffer_test/chat_packet"
...
func (s *ChatServer) OnReqLogin(client *gonet.Client, packet *chat_packet.ReqLogin) {
	...
	sendPacket := &chat_packet.ResLogin{
		SenderId: proto.Int32(s.users[client].sessionId),
		Result: chat_packet.Result_OK.Enum(),
	}
	err := s.proxy.ResLogin(client, sendPacket)
	if err != nil {
		fmt.Errorf("%v", err)
	}
}
...
func (s *ChatServer) OnRead(client *gonet.Client, data interface{}) {
	...
	switch chat_packet.PacketType(packet.GetId()) {
	case chat_packet.PacketType_REQ_LOGIN:
		obj := &chat_packet.ReqLogin{}
		gonet.PacketToProto(packet, obj)
